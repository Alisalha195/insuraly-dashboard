...
import { host } from "../../constants/connection";
...
export const getBusinessStages = () => {
  const {
    ...
    queryKey: ["stages"],
    queryFn: () => fetch(`${host}/business/stages`).then((res) => res.json()),
...
export const getBusinessCommercialTypes = () => {
  const {
    ...
    isLoading,
    isSuccess: isSuccessCommercialTypes,
  } = useQuery({
    queryKey: ["commercial-types"],
    queryFn: () =>
      fetch(`${host}/business/commercialTypes`).then((res) => res.json()),
    refetchInterval: 5000,
    keepPreviousData: true,
    staleTime: 30000,
  });
...
export const getBusinessStatuses = () => {
  const {
    data: statusesData,
    isLoading,
    isSuccess: isSuccessStatusesData,
  } = useQuery({
    queryKey: ["statuses"],
    queryFn: () => fetch(`${host}/business/statuses`).then((res) => res.json()),
    refetchInterval: 5000,
    keepPreviousData: true,
    staleTime: 30000,
  });
