...
const SearchBox = ({ref, isFull, large}) => {
  ...
  return (
    <Box borderWidth={1} borderStyle={'solid'} borderColor={textPrimary} paddingY={large ? "10px" :'2px'} paddingX='4px' borderRadius={9} width={isFull && '100%'} >
      <InputGroup >
         <Flex  className='btn text-[20px]' justifyContent={'center'} paddingY={0}>
            <Flex flexDirection={'column'} justifyContent={'center'} paddingY={0} paddingLeft={1}>
               <LuSearch className='text-[26px]'/> 
            </Flex>
            <Input ref={ref && ref} className='text-[20px]' marginY={0} paddingY='4px' placeholder="Search..." size='xs' border={'none'} outline={'none'} height={'auto'} lineHeight={1}
            />
         </Flex>
      </InputGroup>
    </Box>
